...
- Latest local live verification without `FRED_API_KEY` or `EIA_API_KEY`: 22/22 numeric indicators covered, 1,568 observations, 0 numeric `sample_fallback` rows.
...
Implement Sprint 9 from `docs/open_data_expansion_plan.md`: Historical Chart Layer And Drilldown. Reintroduce historical line charts as the first analytical layer at the top of the static report, with a global view first and drilldown into regional and sector/subsector views. Use existing live and clearly labeled proxy/sample-backed histories, show source/freshness/frequency/data-class/proxy-sample-missing metadata, treat metrics and multiples conservatively, preserve static GitHub Pages HTML/JSON/assets, update coverage/methodology/tests/docs, run local tests and static-site QA, commit/push, dispatch the live workflow, and verify Pages still shows `live_numeric` with 0 numeric `sample_fallback`.
```
