...
Implement Sprint 7 from `docs/open_data_expansion_plan.md`: Proxy Label Cleanup And Coverage Honesty. Rename or replace misleading proxy labels, especially `global_pmi`; add a "Contradicting Evidence" summary to static reports using existing signal components; update framework coverage, methodology, tests, and docs; run local tests and static-site QA; commit/push; manually dispatch the live GitHub Actions workflow; and verify the public Pages report still shows `live_numeric` with 0 numeric `sample_fallback`.
```
