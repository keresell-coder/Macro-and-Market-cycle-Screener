...
Implement Sprint 9 from `docs/open_data_expansion_plan.md`: Credit, Liquidity, And Financial Conditions. Add one or two robust keyless credit/liquidity series first, likely Chicago Fed NFCI through FRED public CSV and possibly one public spread/stress proxy, with source-status rows, freshness metadata, visible fallback behavior, framework coverage updates, tests, static-site QA, commit/push, live workflow dispatch, and Pages verification.
```
