...
Scope:
1. Add one or two robust keyless or configured-key credit/liquidity series first, not a broad fragile catalog.
...
9. Preserve static GitHub Pages HTML/JSON/assets; do not build hosted Streamlit.
10. Run local tests and static-site QA.
11. Commit and push.
12. Dispatch the live GitHub Actions workflow and verify Pages still shows `live_numeric` with 0 numeric `sample_fallback`.
